                    ...
                    <div className='w-1/3 border rounded-lg p-5 my-5 shadow-lg border-[#b9b9b9] bg-white'>
                        ...
                        <form onSubmit={submitHandler} className='my-5 items-stretch'>
                            ...
                            <div name='citydetails' className='flex justify-between gap-4 my-5'>
                                <input name='postalcode' type={'number'} className='border-b-2 border-[#6b6b6b] focus:outline-none w-[4em]' placeholder='PLZ' defaultValue={user.address && user.address.postalcode} required />
                                <input name='city' type={'text'} className='border-b-2 border-[#6b6b6b] focus:outline-none w-full' placeholder='Ort' defaultValue={user.address && user.address.city} />
                            </div>
                            <div name='addressdetails' className='flex justify-between gap-4 my-5'>
                                <input name='street' type={'text'} className='border-b-2 border-[#6b6b6b] focus:outline-none w-5/6' placeholder='Straße' defaultValue={user.address && user.address.street} />
                                <input name='housenumber' type={'text'} className='border-b-2 border-[#6b6b6b] focus:outline-none w-1/6' placeholder='Nr.' defaultValue={user.address && user.address.housenumber} />
                            </div>
                            <input name='phone' type={'tel'} className='border-b-2 border-[#6b6b6b] focus:outline-none w-full mb-3' placeholder='Mobil- / Telefonnummer' defaultValue={user.contact && user.contact.phonenumber} />
                            <div className='flex items-center'>
                                <input name='isActive' type={'checkbox'} className='mr-2' defaultChecked={user.isActive} />
                                <label htmlFor='isActive'>Profil aktiv</label>
                            </div>
                            <div className='flex justify-between my-5 gap-4'>
                                <button name='reset' type='reset' className='border-2 border-red rounded-md px-3 py-1 text-red font-bold hover:bg-red hover:text-white'>Zurücksetzen</button>
                                <button name='submit' type='submit' className='border-2 border-green rounded-md px-3 py-1 text-green font-bold hover:bg-green hover:text-white'>Speichern</button>
                            </div>
                        </form>
                    </div>
